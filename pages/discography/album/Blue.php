<head>
    <title>?Blue?</title>
    <?php include "../../../backend/nav.php" ?> 
    <link rel="stylesheet" href="/css/base.css">
    <link rel="stylesheet" href="/css/discography.css">
    <link rel="stylesheet" href="/css/lyrics.css">
    <link rel="stylesheet" href="/css/meta.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/js-yaml/4.1.0/js-yaml.min.js"></script>
    <script src="/js/lyrics.js"></script>
</head>
<body>
    <div class="main">
        <p>This album does not exist yet, this is just a placeholder of sorts with information based on snippets, speculation, etc...</p>
        <p>Im calling it ?Blue? here cause it has no name yet</p>
        <div class="songs"></div>
        <div class="albumInfo">
            <h2>Album Info</h2>
            <h2>- NAME -</h2>
                <h3>Release Date: xxxx</h3>
                <h3>Songs: xxxx</h3>
                <h3>Total Length: xxxx</h3>
                <h3>Related: <a href="">xxxx</a>, <a href="">xxxx</a></h3>
                <h2>Facts:</h2>
                <h3>xxxx</h3>
                <h3>xxxx</h3>
                <h3>xxxx</h3>
            <img src="\assets\images\placeholder.webp">
        </div>
    </div>
</body>
</html>